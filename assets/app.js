...
// buttão para escutar o clique e executar todas as funções
button.addEventListener('click', () => {
  validateCampo(inputDay.value, inputMonth.value, inputYear.value)
  daysInMonthh(inputMonth.value, inputYear.value, inputDay.value)
})
...
function daysInMonthh(month, year, inputDay) {
  let pasteDate = new Date(year, month, 0).getDate()
  if (inputDay > pasteDate) {
    campos[0].style.border = '1px solid red'
    erro[0].textContent = 'Must be a valid day'
    label[0].style.color = 'red'
  }
